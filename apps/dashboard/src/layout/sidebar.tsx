												...
												<ul className="pt-2 pl-7" role='tablist'>
													{
														menu.selectableChildPath.map((childMenu: ISelectableChildPath, iChild: number) => (
															...
															>
																<Link
																	to={childMenu.path}
																	className={`flex items-center gap-x-3.5 py-1.5 px-2.5 text-sm rounded-md hover:text-blue-500 ${iChild === 0 ? '' : 'mt-1'} ${handleActiveLink([childMenu.path]) ? 'text-blue-600' : 'dark:hover:text-blue-500 dark:text-white'}`}
																>
																	{childMenu.name}
																</Link>
															</li>
