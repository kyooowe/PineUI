...
const Table = <T extends object>({ tableClassName, headClassName, bodyClassName, column, rows }: ITableProps<T>) => {
    ...
    const tableClass = twMerge("text-sm text-left text-gray-500 dark:text-gray-400", tableClassName)
                        ...
                        rows.map((item: T, i: number) => (
                            <tr key={i}>
                                {Object.values(item).map((value: any, innerIndex) => (
                                    <th
                                        key={innerIndex}
                                        className={`px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white ${i === rows.length - 1 && innerIndex === 0 ? 'rounded-bl-lg' : i === rows.length - 1 && innerIndex === rows.length ? 'rounded-br-lg' : ''
                                            }`}>
                                        {value}
                                    </th>
                                ))}
